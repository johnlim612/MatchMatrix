...
def list_players(sort_alpha=False):
    ...
    if sort_alpha:
        ...
        return players
    players = [
        player for player in sorted(playerData, key=lambda x: x["rating"], reverse=True)
    ]
...
def create_teams(players: list):
    ...
    # Sort players by role and then by rating (ascending)
    # include reverse=False for opposite
    sorted_players = sorted(
        player_list, key=lambda x: (x["role"], -x["rating"]), reverse=False
    )
    ...
    for role, players_list in grouped_players.items():
        ...
        team1_stronger = sum(player["rating"] for player in team1) > sum(
        ...
        if len(players_list) >= 2:
            ...
            if len(team1) == len(team2):
                (
                    team2.append(players_list[0])
                    if team1_stronger
                    else team1.append(players_list[0])
                )
                continue
...
def change_player_rating(players, team_rating_diff, team_won: bool):
    ...
    if team_won:
        rating_change = max(
            20, min(rating_change, 200)
        )  # Ensuring rating_increase is between 20 and 200
        for player in players:
            player["rating"] = player["rating"] + rating_change
            player["loss_streak"] = 0
    else:
        rating_change = min(
            -20, max(rating_change, -200)
        )  # Ensuring rating_increase is between -20 and -200
        for player in players:
            player["rating"] = player["rating"] - rating_change
...
def update_players_ratings(winning_player_names, losing_player_names):
    playerData = FileManager.open_player_data()
    losing_players = [
        player for player in playerData if player["name"] in losing_player_names
    ]
    winning_players = [
        player for player in playerData if player["name"] in winning_player_names
    ]
